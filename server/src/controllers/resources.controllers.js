...
import { Branch } from "../models/resources.model.js";
import { StudyMaterial } from "../models/resources.model.js";
import { Year } from "../models/resources.model.js";
...
export const createBranch = asyncHandler(async (req, res) => {
    ...
    if (!BRANCH_NAMES.includes(branch_name)) {
      throw new ApiError(`Invalid branch name. Must be one of: ${BRANCH_NAMES.join(", ")}`, 400);
    }
...
export const createYear = asyncHandler(async (req, res) => {
  ...
  if (!YEAR_NAMES.includes(year_name)) {
    throw new ApiError(`Invalid year name. Must be one of: ${YEAR_NAMES.join(", ")}`, 400);
  }
...
export const getAllYears = asyncHandler(async (req, res) => {
  ...
  res.status(200).json(new ApiResponse(200, years, "Years retrieved successfully"));
});
...
export const getYearById = asyncHandler(async (req, res) => {
  ...
  res.status(200).json(new ApiResponse(200, year, "Year retrieved successfully"));
});
...
export const createStudyMaterial = asyncHandler(async (req, res) => {
    const { title, material_type, branch_id, year_id } = req.body;
  
    if (!title || !material_type || !branch_id || !year_id || !req.file) {
      throw new ApiError("All fields (title, type, branch, year, file) are required", 400);
    }
  
    const fileUrl = await uploadImageToCloudinary(req.file.path, "study_materials");
    ...
    res.status(201).json(new ApiResponse(201, studyMaterial, "Study material uploaded successfully"));
  });
  ...
  export const getStudyMaterialById = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(new ApiResponse(200, material, "Study material retrieved successfully"));
  });
  ...
  export const updateStudyMaterial = asyncHandler(async (req, res) => {
    ...
    await material.update({ title, material_type, material_url: fileUrl, branch_id, year_id });
  
    res.status(200).json(new ApiResponse(200, material, "Study material updated successfully"));
  });
